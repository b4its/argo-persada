...
```bash
docker exec -it argo-db mysql -u root
```

### untuk masuk ke terimal container apache
```bash
docker exec -it argo-php-fpm bash
```

### untuk solo command
```bash
# Jika pakai Nginx
docker exec -it argo-php-fpm php artisan serve
docker compose exec php-fpm composer install
# untuk melihat log seperti php artisan serve
docker compose logs -f nginx php-fpm
# Jika pakai Apache
docker exec -it argo-apache php artisan serve
```
